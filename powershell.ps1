...
$SQL = @"
mysql -h $MySqlHostname -u $MySqlUserName -p$MySqlPassword $MySQLDatabase -e "SELECT * FROM $MySqlTable ORDER BY status DESC, hostname ASC ;" 2>/dev/null | ConvertFrom-Csv -delimiter ``t
"@

$SQL = Invoke-Expression $SQL 
$SQL | Select hostname,type,port,status,lastupdate_utc | Export-CSV query.csv -Force
